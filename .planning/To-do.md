...
- **皮膚 AI 生成**
  ...
  - **2026-07 狀態**：右鍵選單項**暫時隱藏**（實際跑過可行，但預設參數生成結果崩壞/很恐怖，待 IP_SCALE/提示詞/步數調校穩定後再開回）。程式碼與 `/generate_skin` 端點保留。
  - **2026-07 移除**：早期「AI 變身」單張 img2img（右鍵）+ 後端 `/transform` 端點已刪（QLabel 時代產物，Live2D 主線下失效，功能亦被整套生成涵蓋）。
